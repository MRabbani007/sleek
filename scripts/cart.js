    ...
    cartHTML += `
          <div class="row p-0" id="cart-item-${cart[i].productId}">
            <div class="card border-1 my-3 p-0">
              <div class="card-header text-success fw-bold">Delivery Date: ${
                cart[i].deliveryDate
              }</div>
              <div class="card-body p-3">
                <div class="row justify-content-between p-0 m-0 g-0 gap-0">
                  <div class="col-2 m-0"><img class="cart-photo" src="../${
                    product.image
                  }"></div>
                  <div class="col-lg-7 col-sm-5 m-0">
                    <div class="h5">${product.name}</div>
                    <div class="card-item text-danger">$${(
                      cart[i].unitPrice / 100
                    ).toFixed(2)}</div>
                    <div class=""> 
                      <div class="card-item mx-1">Quantity: <span id="item-qty-${
                        cart[i].productId
                      }">${cart[i].quantityOrder}</span></div>
                      </div>
                      
                      <div class="mx-1 btn btn-warning" onclick="removeFromCart('${
                        cart[i].productId
                      }')">Delete</div>

                      <div class="mx-1 btn"><a href="#edit-item-${
                        cart[i].productId
                      }" class="btn btn-primary  dropdown-toggle" data-bs-toggle="collapse">Update</a>
                      
                      <div id="edit-item-${cart[i].productId}" class="collapse">
                        <input class="form-control" type="number" id="edit-qty-${
                          cart[i].productId
                        }" value="${cart[i].quantityOrder}">
                        <button class="form-control btn btn-success d-inline" onclick="applyEdit('${
                          cart[i].productId
                        }')">Apply</button>
                      </div>
...
function genSummary() {
  ...
  document.getElementById("items-shipping-price").innerHTML =
    shippingPrice.toFixed(2);
  document.getElementById("items-total-before-tax").innerHTML = (
    orderTotal + shippingPrice
  ).toFixed(2);
  document.getElementById("items-estimated-tax").innerHTML = tax.toFixed(2);
  document.getElementById("items-total-price").innerHTML = (
    orderTotal +
    shippingPrice +
    tax
  ).toFixed(2);
