...
function generateItemHTML(productID) {
  ...
  let variantValue =
    product.variants[0][findProp(productIndex, "image")].value[0][0];

  // Generate HTML for left Images
  const { imageHTML, imgExpanded } = genThumbnailHTML(
    productIndex,
    variantProp,
    variantValue
  );
  ...
  product.variants[0].forEach((property) => {
    if (property.prop === "size") {
      ...
      productProps.push({
        ...
        value: genMapHTML(
          productIndex,
          findProp(productIndex, "color"),
          findProp(productIndex, "image"),
          0
        ),
      });
    }
  ...
  <div class="row">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <div class="d-flex align-center">
            ...
            <!--
            <img class="img-ratings" src='${getRatingsImage(
    ...
    <div class="col-lg-2">
      <div class="card p-2">
        ...
        ).toFixed(2)} Shipping</p>
        <div class="btn btn-warning my-3" onclick="addToCart('${
          product.id
        }')">Add to Cart</div>
...
function selectedChange(productIndex, changedProperty) {
  ...
  try {
    document.querySelector(".style-cont").innerHTML = genBlockHTML(
      productIndex,
      findProp(productIndex, "style"),
      selectedVariantIndex
    );
  } catch {}
  try {
    document.querySelector(".color-cont").innerHTML = genMapHTML(
      productIndex,
      findProp(productIndex, "color"),
      findProp(productIndex, "image"),
      selectedVariantIndex
...
function genBlockHTML(productIndex, propertyIndex, selectedVariantIndex = 0) {
  ...
  let selectedVariant =
    productsNew[productIndex].variants[selectedVariantIndex];
  ...
  for (let i = 0; i < variants.length; i++) {
    ...
    for (let j = 0; j < variants[i].length; j++) {
      if (variants[i][j].prop === propName) {
        ...
        if (
          Object.prototype.toString.call(variants[i][j].value) ===
          "[object Array]"
        ) {
...
function changeThumbs(productIndex, searchImage) {
  let { imageHTML, imgExpanded } = genThumbnailHTML(
    productIndex,
    "image",
    searchImage
  );
...
function genThumbnailHTML(productIndex, propName, propValue) {
  // Generate Image HTML
  let imageHTML = "";
  let { variantIndex, propIndex, valueIndex } = findVariant(
    productIndex,
    propName,
    propValue
  );
  let product = productsNew[productIndex];
  product.variants[variantIndex][propIndex].value[valueIndex].forEach(
    (image) => {
      imageHTML += `
...
function carouselRight() {
  ...
  const carWidth = document
    .querySelector(".MultiCarousel-inner")
